...
export async function enforceUsage(userId: string, plan: PlanId | "none", quota: QuotaKey): Promise<UsageResult> {
  ...
  if (upstashConfigured()) {
    try {
      // Читаем текущее значение; если место есть — INCR и ставим срок жизни до конца месяца.
      const cur = Number((await upstash([["GET", key]]))[0]?.result ?? 0);
      ...
      const used = Number(r[0]?.result ?? cur + 1);
