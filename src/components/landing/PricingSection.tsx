...
const PLANS = [
  { name: "Starter", price: { monthly: 0, annual: 0 }, description: "Perfect for exploring your first business idea.", cta: "Start Free", features: ["3 strategy reports / month","All 8 AI executives","PDF export","Basic financial model","Email support"], popular: false, gradient: "from-slate-600 to-gray-600" },
  { name: "Pro", price: { monthly: 49, annual: 39 }, description: "For founders building and launching their business.", cta: "Start Pro Trial", features: ["Unlimited strategy reports","All 8 AI executives","Premium PDF export","Advanced financial models","Competitive intelligence","Priority processing","Slack integration","Priority support"], popular: true, gradient: "from-violet-600 to-blue-600" },
  { name: "Agency", price: { monthly: 149, annual: 119 }, description: "For agencies and consultants with multiple clients.", cta: "Start Agency Trial", features: ["Everything in Pro","Unlimited client projects","White-label reports","Custom branding","Team collaboration","API access","Dedicated account manager","SLA guarantee"], popular: false, gradient: "from-amber-600 to-orange-600" },
];
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-3 mt-8 p-1 rounded-xl glass border border-white/[0.08]">
            <button onClick={() => setAnnual(false)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${!annual ? "bg-violet-600 text-white shadow-lg shadow-violet-500/25" : "text-white/40 hover:text-white"}`}>Monthly</button>
            <button onClick={() => setAnnual(true)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${annual ? "bg-violet-600 text-white shadow-lg shadow-violet-500/25" : "text-white/40 hover:text-white"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PLANS.map((plan) => (
            <div key={plan.name} className={`relative rounded-2xl p-px overflow-hidden ${plan.popular ? "shadow-2xl shadow-violet-500/20" : ""}`}>
              ...
              <div className={`relative rounded-[15px] p-6 flex flex-col h-full ${plan.popular ? "bg-[#0d0d14]" : "bg-[#0f0f0f] border border-white/[0.07]"}`}>
                ...
                {plan.popular && <div className="inline-flex self-start px-2.5 py-1 rounded-full bg-violet-500/20 border border-violet-500/30 text-violet-300 text-[10px] font-semibold uppercase tracking-wider mb-4">Most Popular</div>}
                <div className="mb-6">
