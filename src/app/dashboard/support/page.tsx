...
const QUICK_ACTIONS = [
  { icon: BookOpen,    label: "Документация",   desc: "Гайды и туториалы",          color: "#6366f1", rgb: "99,102,241" },
  { icon: Play,        label: "Видеоуроки",     desc: "Обучающие видео",             color: "#4f46e5", rgb: "79,70,229" },
  { icon: Bot,         label: "AI Помощник",    desc: "Мгновенные ответы",           color: "#10b981", rgb: "16,185,129" },
  { icon: MessageSquare,label: "Live Chat",     desc: "Онлайн прямо сейчас",        color: "#8b5cf6", rgb: "139,92,246" },
  { icon: Ticket,      label: "Создать тикет",  desc: "Официальное обращение",       color: "#f59e0b", rgb: "245,158,11" },
  { icon: Activity,    label: "Статус системы", desc: "Все сервисы работают",        color: "#10b981", rgb: "16,185,129" },
  { icon: Calendar,    label: "Демо-звонок",    desc: "Запланировать встречу",       color: "#ef4444", rgb: "239,68,68" },
  { icon: Users,       label: "Сообщество",     desc: "Форум и Discord",             color: "#4f46e5", rgb: "79,70,229" },
];

const KB_CATEGORIES = [
  { icon: Zap,        label: "Начало работы",    articles: 12, color: "#6366f1" },
  { icon: Layers,     label: "Создание стратегии",articles: 8,  color: "#4f46e5" },
  { icon: BarChart2,  label: "Аналитика",         articles: 15, color: "#10b981" },
  { icon: Bot,        label: "AI Агенты",          articles: 11, color: "#8b5cf6" },
  { icon: TrendingUp, label: "Финансовые модели",  articles: 7,  color: "#f59e0b" },
  { icon: ExternalLink,label: "Интеграции",        articles: 9,  color: "#ef4444" },
  { icon: Key,        label: "API",                articles: 6,  color: "#4f46e5" },
  { icon: FileText,   label: "Отчёты",             articles: 10, color: "#6366f1" },
  { icon: Settings,   label: "Настройки",          articles: 8,  color: "#10b981" },
  { icon: Shield,     label: "Безопасность",       articles: 5,  color: "#ef4444" },
];
...
const STATUS_STYLE: Record<string, { color: string; bg: string; dot: string }> = {
  "Решено":   { color: "#10b981", bg: "rgba(16,185,129,0.1)",  dot: "#10b981" },
  "В работе": { color: "#f59e0b", bg: "rgba(245,158,11,0.1)",  dot: "#f59e0b" },
  "Ожидает":  { color: "#4f46e5", bg: "rgba(79,70,229,0.1)", dot: "#4f46e5" },
};
...
export default function SupportPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#07090F", position: "relative", overflow: "hidden" }}>
      {/* Ambient */}
      <div style={{ position: "fixed", top: -200, left: "50%", transform: "translateX(-50%)", width: 700, height: 400, borderRadius: "50%", background: "radial-gradient(ellipse, rgba(99,102,241,0.08) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: -100, right: -100, width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(16,185,129,0.04) 0%, transparent 70%)", pointerEvents: "none" }} />

      {/* ── HERO ── */}
      <div style={{ padding: "52px 40px 40px", textAlign: "center", position: "relative" }}>
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 14px", borderRadius: 20, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.18)", marginBottom: 20 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#10b981", boxShadow: "0 0 8px #10b981" }} />
            <span style={{ fontSize: 11, color: "#10b981", fontWeight: 700 }}>Support Online · Среднее время ответа: 1.4 ч</span>
          </div>
          <h1 style={{ fontSize: 38, fontWeight: 900, color: "#fff", letterSpacing: "-1px", lineHeight: 1.1, marginBottom: 12 }}>
            Центр поддержки<br />
            <span style={{ background: "linear-gradient(135deg, #6366f1, #4f46e5)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>AI Executive Board</span>
          </h1>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 10 }}>
            <button onClick={() => setTicketOpen(true)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 20px", borderRadius: 11, fontSize: 13, fontWeight: 700, border: "none", background: "linear-gradient(135deg, #6366f1, #4f46e5)", color: "#fff", cursor: "pointer", boxShadow: "0 4px 20px rgba(99,102,241,0.35)" }}>
      ...
      <div style={{ padding: "0 40px 60px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 16, marginBottom: 16 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, background: "linear-gradient(90deg, transparent, #10b98180, transparent)" }} />
            <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 20, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.15)" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center" }}>
                    {AI_QUICK.map(q => (
                      <button key={q} onClick={() => sendAI(q)}
                        style={{ padding: "5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 500, border: "1px solid rgba(99,102,241,0.25)", background: "rgba(99,102,241,0.08)", color: "#8b5cf6", cursor: "pointer" }}>{q}</button>
                    ))}
                  </div>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {aiMessages.map((m, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                      <div style={{ maxWidth: "80%", padding: "9px 13px", borderRadius: m.role === "user" ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: m.role === "user" ? "rgba(99,102,241,0.2)" : "rgba(16,185,129,0.08)", border: m.role === "user" ? "1px solid rgba(99,102,241,0.3)" : "1px solid rgba(16,185,129,0.15)", fontSize: 12, color: "rgba(255,255,255,0.8)", lineHeight: 1.5 }}>
                        {m.text}
                      </div>
                    </div>
                  ))}
            ...
            <div style={{ padding: "12px 16px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", gap: 8 }}>
              ...
              <button onClick={() => sendAI("")}
                style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(16,185,129,0.15)", border: "1px solid rgba(16,185,129,0.25)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              style={{ borderRadius: 18, padding: "18px 20px", background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.1)", flex: 1, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 10, padding: "3px 8px", borderRadius: 20, background: "rgba(16,185,129,0.12)", color: "#10b981", fontWeight: 700 }}>Всё работает</span>
              </div>
                ...
                {SERVICES.map((s, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      ...
                      <span style={{ fontSize: 10, color: s.status === "ok" ? "#10b981" : "#f59e0b", fontWeight: 600 }}>{s.status === "ok" ? "Работает" : "Замедление"}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <c.icon size={12} style={{ color: "#6366f1" }} />
                    </div>
                    <div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, background: "linear-gradient(90deg, transparent, #6366f160, transparent)" }} />
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            ...
            <button style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 600, color: "#6366f1", background: "transparent", border: "none", cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            style={{ borderRadius: 20, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", overflow: "hidden", position: "relative" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, background: "linear-gradient(90deg, transparent, #4f46e560, transparent)" }} />
            ...
            <div style={{ padding: "8px 8px" }}>
              {POPULAR_ARTICLES.map((a, i) => (
                <motion.button key={i} whileHover={{ background: "rgba(255,255,255,0.03)" }} transition={{ duration: 0.15 }}
                  ...
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#6366f1", flexShrink: 0, opacity: 0.7 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      {a.new && <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 4, background: "rgba(16,185,129,0.1)", color: "#10b981", fontWeight: 700 }}>NEW</span>}
                    </div>
                  </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <div style={{ padding: "20px 22px 14px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setTicketOpen(true)}
                style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 8, fontSize: 11, fontWeight: 700, border: "none", background: "rgba(99,102,241,0.12)", color: "#8b5cf6", cursor: "pointer" }}>
                <Plus size={11} />Новый
              </button>
            </div>
            <div style={{ padding: "8px 8px" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
            {FAQ_CATS.map(c => (
              <button key={c} onClick={() => setFaqCat(c)}
                style={{ padding: "5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: `1px solid ${faqCat === c ? "rgba(99,102,241,0.4)" : "rgba(255,255,255,0.07)"}`, background: faqCat === c ? "rgba(99,102,241,0.12)" : "transparent", color: faqCat === c ? "#8b5cf6" : "rgba(255,255,255,0.35)", cursor: "pointer", transition: "all 0.15s" }}>{c}</button>
            ))}
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {filteredFAQ.map((faq, i) => (
              <div key={i} style={{ borderRadius: 12, border: `1px solid ${faqOpen === i ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.05)"}`, background: faqOpen === i ? "rgba(99,102,241,0.05)" : "transparent", overflow: "hidden", transition: "all 0.2s" }}>
                <button onClick={() => setFaqOpen(faqOpen === i ? null : i)}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                    <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 5, background: "rgba(99,102,241,0.1)", color: "#8b5cf6", fontWeight: 700, flexShrink: 0 }}>{faq.cat}</span>
                  ...
                  <motion.div animate={{ rotate: faqOpen === i ? 180 : 0 }} transition={{ duration: 0.2 }}>
                    <ChevronDown size={14} style={{ color: faqOpen === i ? "#6366f1" : "rgba(255,255,255,0.25)", flexShrink: 0 }} />
                  </motion.div>
                </button>
                <AnimatePresence>
                  {faqOpen === i && (
              ...
              {ticketSent ? (
                <div style={{ padding: "60px 40px", textAlign: "center" }}>
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }}
                    style={{ width: 64, height: 64, borderRadius: "50%", background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.25)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
                ...
                <>
                  ...
                  <div style={{ padding: "22px 28px" }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <button onClick={sendTicket} style={{ flex: 2, padding: "10px", borderRadius: 11, fontSize: 12, fontWeight: 700, border: "none", background: "linear-gradient(135deg, #6366f1, #4f46e5)", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
