...
function AgentPanel({ letter, name, subtitle, color, score, opinion, delay = 0, agentId, projectContext }:
  ...
  const streamOpinion = useCallback(async () => {
    ...
    const message = `${projectContext}\n\nДай развёрнутую экспертную оценку этого проекта со своей профессиональной точки зрения (${subtitle}). Ровно 7–10 конкретных предложений: что сильно, что слабо, ключевые риски и чёткие рекомендации по твоей зоне. По-русски, обычный текст без списков и markdown.`;
