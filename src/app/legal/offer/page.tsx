...
// подписки на текущем этапе отключены, приём платежей не ведётся — поэтому
...
const SECTIONS: Section[] = [
  {
    ...
    blocks: [
      { kind: "p", text: "Настоящий документ является официальным предложением (публичной офертой) владельца онлайн-платформы Vertlix AI (далее — «Исполнитель», «Сервис») заключить договор безвозмездного предоставления доступа к Сервису на изложенных ниже условиях с любым дееспособным лицом, которое примет это предложение (далее — «Заказчик», «Пользователь»)." },
      { kind: "p", text: "Акцептом (принятием) оферты считается регистрация аккаунта в Сервисе либо начало использования его функций. С момента акцепта договор считается заключённым в электронной форме без подписания бумажного экземпляра." },
  ...
  },
  {
    ...
    blocks: [
      { kind: "p", text: "Исполнитель безвозмездно предоставляет Заказчику удалённый доступ к функциональности Сервиса: аналитическим инструментам на базе искусственного интеллекта для анализа бизнес-идей, генерации стратегий, планов, финансовых и рыночных оценок." },
  ...
  {
    id: "tariffs", num: "3", title: "Стоимость услуг",
    blocks: [
      { kind: "callout", text: "На текущем этапе доступ к Сервису предоставляется бесплатно. Платные подписки не подключены, оплата не взимается, платёжные данные Пользователя не запрашиваются и не обрабатываются." },
      { kind: "p", text: "Исполнитель вправе ввести платные тарифы в будущем. В этом случае условия оплаты публикуются отдельной редакцией настоящей оферты, а Пользователь уведомляется заранее по email или внутри Сервиса. Введение платных тарифов не распространяется на прошедший период использования." },
      { kind: "p", text: "Использование Сервиса может быть ограничено технически (например, ограничение числа запросов к AI-инструментам) для стабильной работы и защиты от злоупотреблений." },
    ],
  },
  {
    ...
    blocks: [
      ...
      { kind: "list", intro: "Заказчик обязуется:", items: [
        ...
        "не предпринимать действий, создающих чрезмерную нагрузку на Сервис или направленных на обход технических ограничений.",
      ] },
    ],
  },
  {
    ...
    blocks: [
      ...
      { kind: "p", text: "Поскольку Сервис предоставляется безвозмездно, Исполнитель не несёт имущественной ответственности за возможные убытки, связанные с использованием или невозможностью использования Сервиса, за исключением случаев, когда такая ответственность прямо установлена законом и не может быть ограничена." },
  ...
  },
  {
    ...
    blocks: [
      { kind: "p", text: "Договор действует с момента акцепта до удаления аккаунта любой из сторон. Исполнитель вправе изменять условия оферты; новая редакция публикуется на этой странице и применяется к отношениям, возникшим после её публикации. О существенных изменениях Исполнитель уведомляет заранее." },
  ...
  },
  {
    ...
    blocks: [
      { kind: "p", text: "Сервис предоставляется бесплатно и не связан с приёмом платежей, поэтому платёжные и регистрационные реквизиты не указываются. Для любых вопросов, обращений и претензий по работе Сервиса используйте контактный email — мы отвечаем на все обращения." },
      { kind: "list", items: [
        "Сервис: Vertlix AI",
        `Email для связи: ${CONTACT_EMAIL}`,
      ] },
      { kind: "callout", text: "Редакция оферты от июля 2026 года. При запуске платных тарифов эта страница будет дополнена полными реквизитами исполнителя и условиями оплаты." },
    ],
  },
];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#05060A", color: "#fff" }}>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        <p style={{ fontSize: 15, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", margin: "0 0 36px" }}>Договор безвозмездного предоставления доступа к платформе Vertlix AI. Регистрируясь в Сервисе, вы принимаете условия этого договора.</p>
